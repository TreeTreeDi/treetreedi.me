...
- **Open article.** Choose the newest listed title. Run `control-treetreedi browser click --role link --name "Claude Code 记忆系统工程指南：CLAUDE.md 与 Auto Memory"`. The URL is `/posts/claude-code-memory-system` and the article heading matches that title.
...
- Post titles are long and must be quoted exactly. The first visible title is the newest by frontmatter `date`.
